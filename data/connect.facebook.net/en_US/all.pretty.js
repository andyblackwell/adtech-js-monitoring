/*1547489042,,JIT Construction: v4682139,en_US*/
				...
				with (this) {
											...
											l.add(
												...
												m(
													function(a) {
														...
														if (
															e !== "native" &&
															!(r.test(location.hostname) || r.test(a.origin))
														)
															return;
														if (typeof d !== "string") {
															h.warn(
						...
						function(
							...
							function K(a) {
								...
								if (d || f) {
									j = "unknown";
									E(null, j);
									a && a({ authResponse: null, status: j });
									return;
								}
								var t = G(a, B, "login_status");
								d = new v(l.resolve("www") + "/connect/ping")
							...
							function a(a, b) {
								...
								if (a)
									if (!b && L === "loaded") {
										var c = { authResponse: F(), status: s.getLoginStatus() };
										a(c);
										return;
					...
					__d(
						...
						function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q) {
							...
							function C(a, b, c, d) {
								...
								x++;
								var f = babelHelpers["extends"]({}, t, c);
								f.pretty = f.pretty || 0;
								f = q(f);
								...
								h.length > 0 &&
									((a = o.appendToUrl(a, g)),
									ES(h, "forEach", !0, function(a) {
										return delete f[a];
									}));
								f.transport
									? ((g = [f.transport]), delete f.transport)
									: (g = w);
								for (var h = 0; h < g.length; h++) {
									var i = e[g[h]],
										m = ES("Object", "assign", !1, {}, f);
											...
											e = ES(
												...
												ES(
													...
													a.params.response_type
														? j(a.params.response_type.split(","))
														: {},
							...
							var M = {
								...
								prepareCall: function(a, b) {
									...
									var c = a.method.toLowerCase(),
										d = Object.prototype.hasOwnProperty.call(M.Methods, c)
											? ES("Object", "assign", !1, {}, M.Methods[c])
											: {},
										e = l(),
										f = !0;
									...
									(c === "share" || c === "share_open_graph") &&
										((a.mobile_iframe =
											D.mobile() && (a.mobile_iframe || a.iframe_test)),
									...
									f = {
										...
										url:
											i.resolve(a.display == "touch" ? "m" : "www") +
											"/" +
											d.url,
										params: a,
										name: c,
									...
									b = c(f.params);
									!(f.id in M._defaultCb) &&
										!("next" in f.params) &&
										!("redirect_uri" in f.params) &&
										(f.params.next = M._xdResult(f.cb, f.id, b, !0));
									(b === "parent" || b === "opener") &&
										ES(
											"Object",
											"assign",
								...
								prepareParams: function(a) {
									...
									a.params.kid_directed_site =
										B.getKidDirectedSite() || a.params.kid_directed_site;
									a.params = k(a.params);
									var b = h.encode(a.params);
									!D.nativeApp() && M.urlTooLongForIE(a.url + "?" + b)
										? (a.post = !0)
										: b && (a.url += "?" + b);
									return a;
								},
								...
								getDisplayMode: function(a, b) {
									...
									if (
										...
										return b.display;
									var c =
										B.isEnvironment(B.ENVIRONMENTS.CANVAS) ||
										B.isEnvironment(B.ENVIRONMENTS.PAGETAB);
									...
									return a.connectDisplay && !c
										? a.connectDisplay
										: b.display || (M.canIframe(b) ? "dialog" : "popup");
								},
								canIframe: function(a) {
									if (B.getAccessToken()) return !0;
									return D.mobile() && B.getLoggedIntoFacebook()
										? !!a.mobile_iframe
										: !1;
								},
								getXdRelation: function(a) {
								...
								popup: function(a) {
									...
									if (!f) {
										v("popup_blocker_scribe_logging", !0) &&
											C.log("jssdk_error", {
												appId: B.getClientID(),
								...
								touch: function(a) {
									a.params && a.params.in_iframe
										? a.ui_created
											...
											: M.iframe(a)
										: D.nativeApp() && !a.ui_created
											? ((a.frame = a.id),
											  y.onready(function() {
													M.setLoadedNode(
														a,
														y.open(a.url + "#cb=" + a.frameName),
														"native"
													);
											  }),
											  M._popupMonitor())
											: a.ui_created || M.popup(a);
								},
								async: function(a) {
									__p && __p();
									...
									H(a.params) || I(a.params)
										? ((a.params.method = "oauth"),
										  ...
										  t.remote.showDialog(a.params, b))
										: A.remote.showDialog(a.params, b);
								},
								native: function(a) {
									z(a.params);
								},
								...
								_popupMonitor: function() {
									...
									for (var b in M._loadedNodes)
										if (
											Object.prototype.hasOwnProperty.call(M._loadedNodes, b) &&
											b in M._defaultCb
										) {
								...
								_xdRecv: function(a, b) {
									...
									if (c)
										if (c.close)
											...
											r.containsCss(c, "FB_UI_Hidden")
												? setTimeout(function() {
														c.parentNode.parentNode.removeChild(c.parentNode);
												  }, 3e3)
												: r.containsCss(c, "FB_UI_Dialog") && q.remove(c);
						...
						function(a, b, c, d, e, f, g, h, i, j, k, l, m, n) {
							...
							function q(a, b) {
								...
								if (b.href != null) {
									c = new n(b.href);
									c.getProtocol() ||
										(b.href = c.setProtocol("http").toString());
								}
								return b;
							}
							a = g.extend({
								...
								constructor: function(a, b, c, d) {
									g.addCss(a, "fb_comments_count_zero"),
										...
										this.subscribe("xd.comment_count", function(b) {
											...
											g.html(
												a,
												i('<span class="fb_comments_count">%s</span>', b.count)
											);
					...
					__d(
						...
						function(a, b, c, d, e, f, g) {
							...
							function a(a, b, c, d) {
								...
								return { subscribe: function() {}, process: function() {} };
							}
							e.exports = a;
						},
						null
					...
					__d(
						...
						function(a, b, c, d, e, f, g, h, i, j, k, l) {
							...
							ES(l.getSDKUrl(), "indexOf", !0, "customerchat") !== -1
								? delete m.customerchat
								: (m.customerchat = b("sdk.XFBML.CustomerChatWarning"));
	...
	new Image().src =
		...
		encodeURIComponent(
			'{"error":"LOAD", "extra": {"name":"' +
				...
				'","revision":"4682139","namespace":"FB","message":"' +
				e.message +
				'"}}'
		);
}
